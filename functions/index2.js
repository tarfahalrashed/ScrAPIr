...
const resolvers = {
  Query: {
    videos: () => {
      return fetch(`${baseURL}/videos`).then(res => res.json())
    },
    video: (parent, args) => {
      const { id } = args
      return fetch(`${baseURL}/video/${id}`).then(res => res.json())
    },
    // posts: () => {
...
//   return fetchResponseByURL('/videos?search=cats').then(json => json.people);
// }

// function fetchVideoByURL(relativeURL) {
...
//       resolve: video => video.friends.map(fetchVideoByURL),
//     },
//   }),
// });
...
//       },
//       resolve: (root, args) => fetchVideoByURL(`/video/${args.id}/`),
//     },
//   }),
// });
